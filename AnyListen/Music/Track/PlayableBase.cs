...
using AnyListen.Music.MusicCover;
...
namespace AnyListen.Music.Track
{
        ...
        IMusicInformation
    {
        ...
        public string DisplayText => !string.IsNullOrEmpty(Artist) && AnyListenSettings.Instance.Config.ShowArtistAndTitle
            ? $"{Artist} - {Title}"
            : Title;
        ...
        public virtual async void Unload()
        {
            if (Image != null)
            {
                ...
                try
                {
                    await Task.Delay(100, _disposeImageCancellationToken.Token); //Some animations need that
                }
